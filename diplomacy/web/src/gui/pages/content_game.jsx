...
import playIcon from "../../png/play.png";
import fastForward from "../../png/ff.png";
...
export class ContentGame extends React.Component {
  ...
  notifiedNewGameMessage(networkGame, notification) {
    ...
    else ++messageHighlights[protagonist];
  ...
  onRemoveOrder(powerName, order) {
    ...
    if (orders[powerName]) {
      console.log("Order L", order.loc);
      console.log("Order in Orders:", orders[powerName][order.loc]);
    }
    if (
  ...
  __change_past_phase(newPhaseIndex) {
    console.log("Changing to phase index:", newPhaseIndex);
    ...
    return (
      <div className={"panel-messages"} key={"panel-messages"}>
        ...
        <Tabs
          menu={tabNames}
          titles={titles}
          onChange={this.onChangeTabPastMessages}
          active={currentTabId}
        >
    ...
    return (
      <div id="current-map" key="current-map" className="map-container">
          ...
          <div className="bandeau-container">
            {Object.keys(gameEngine.powers)
              ...
              .map((adversaryPowerName) => (
                <BandeauAdversaire
                  key={adversaryPowerName}
                  powerName={adversaryPowerName}
                  troopCount={
                    gameEngine.powers[adversaryPowerName].units.length
                  }
                  color={POWER_COLORS[adversaryPowerName] || "#95a5a6"}
                  controller={
        ...
        {this.state.showChatWindow && (
          <ChatWindow
            controlledPower={powerName}
            recipientPower={this.state.chatRecipient}
            sendMessage={(recipient, message) =>
              this.sendMessage(this.props.data.client, recipient, message)
            }
            renderPastMessages={() =>
            ...
            textColors={{
              ...
              recipientPower:
                POWER_COLORS[this.state.chatRecipient] || "#95a5a6",
            }}
          />
        )}
  ...
  __form_phases(pastPhases, phaseIndex) {
    return (
      <form key={1} className="form-inline mb-4">
        {/*
                <div className="custom-co</div>ntrol-inline">
        ...
        <div className="form-group d-flex align-items-center">
          {/* Bouton Decrement */}
          <PhaseChangeButton
            targetPhase={pastPhases[0]}
            onChangePhase={() => this.displayFirstPastPhase()}
            disabled={phaseIndex === 0}
            imageSrc={fastForward} // Remplacez par l'icône appropriée
            altText="First Phase"
            className="inverted"
          />
          <PhaseChangeButton
            targetPhase={pastPhases[phaseIndex - 1]}
            onChangePhase={this.onDecrementPastPhase}
            disabled={phaseIndex === 0}
            imageSrc={playIcon}
            altText="Decrement Phase"
            className="inverted"
          />
          {/* Select */}
          <select
            className="custom-select-styled mx-3"
          ...
          <PhaseChangeButton
            targetPhase={pastPhases[phaseIndex + 1]}
            onChangePhase={this.onIncrementPastPhase}
            disabled={phaseIndex === pastPhases.length - 1}
            imageSrc={playIcon}
            altText="Increment Phase"
          />
          <PhaseChangeButton
            targetPhase={pastPhases[pastPhases.length - 1]}
            onChangePhase={() => this.displayLastPastPhase()}
            disabled={phaseIndex === pastPhases.length - 1}
            imageSrc={fastForward} // Remplacez par l'icône appropriée
            altText="Current Phase"
          />
        </div>
        {/*
  ...
  render() {
    ...
    /*
        ...
        />
        <div>
            currentPowerName: {currentPowerName}, phase: {engine.phase}, type: {phaseType}
        </div>
        {this.renderTabCurrentPhase(
            true, // toDisplay
            engine,
            this.getCurrentPowerName(),
            null, // orderType
            [], // orderPath
            this.getCurrentPowerName(),
            null // currentTabOrderCreation
        )}
    </main>
        );
        */
