...
export class Page extends React.Component {
  ...
  loadGameFromDisk() {
    return loadGameFromDisk()
      .then((game) =>
        this.load(
          `game: ${game.game_id}`,
          <ContentGame channel={this.channel} data={game} />,
          {
  ...
  __disconnect(error) {
    ...
    this.channel = null;
    this.availableMaps = null;
    const message = Page.wrapMessage(
      error ? `${error.toString()}` : `Disconnected from channel and server.`
    );
  ...
  logout() {
    ...
    if (this.channel) {
      return this.channel
        ...
        .catch((error) =>
          this.error(`Error while disconnecting: ${error.toString()}.`)
        );
    } else {
    ...
    if (this.state.myGames.hasOwnProperty(gameID)) {
      ...
      if (game.client) {
        return game.client
          ...
          .catch((error) =>
            this.error(`Error when leaving game ${gameID}: ${error.toString()}`)
          );
      }
    } else {
    ...
    if (game) {
      if (game.client) {
        return game.client
          ...
          .catch((error) =>
            this.error(
              `Error when deleting game ${gameID}: ${error.toString()}`
            )
          );
      } else {
        return this.channel
          ...
          .catch((error) =>
            this.error(
              `Error when deleting game after joining it (${gameID}): ${error.toString()}`
            )
          );
      }
    }
  ...
  disconnectGame(gameID) {
    ...
    if (game) {
      ...
      return this.channel
        ...
        .catch((error) =>
          this.error(`Error while leaving game ${gameID}: ${error.toString()}`)
        );
    }
    return null;
  }
  ...
  addToMyGames(game) {
    ...
    return this.setState(this._add_to_my_games(game)).then(() =>
      this.loadGames()
    );
  }
        ...
        <div className="page container-fluid" id={this.state.contentName}>
          <div className={"top-msg row"}>
            <div
              ...
              className={
                "col-sm-4 msg success " +
                (this.state.success ? "with-msg" : "no-msg")
              }
            ...
            <div
              ...
              className={
                "col-sm-4 msg info " + (this.state.info ? "with-msg" : "no-msg")
              }
              onClick={() => this.info()}
            >
              {infoMessage}
            ...
            <div
              ...
              className={
                "col-sm-4 msg error " +
                (this.state.error ? "with-msg" : "no-msg")
              }
              onClick={() => this.error()}
            >
              {errorMessage}
