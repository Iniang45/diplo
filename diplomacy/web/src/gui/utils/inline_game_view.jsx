...
export class InlineGameView {
  ...
  joinGame(formData) {
    const form = {
      ...
      registration_password:
        formData[`registration_password_${this.game.game_id}`],
    };
    ...
    this.page.channel
      ...
      .catch((error) => {
        this.page.error(
          "Error when joining game " + this.game.game_id + ": " + error
        );
      });
  }
    ...
    if (this.game.role) {
      ...
      return (
        <div className={"games-form"}>
          <Button
            key={"button-show-" + this.game.game_id}
            title={"show"}
            onClick={this.showGame}
          />
          <Button
            key={"button-leave-" + this.game.game_id}
            title={"leave"}
  ...
  getActionButtons() {
    ...
    if (
      [STRINGS.MASTER_TYPE, STRINGS.OMNISCIENT_TYPE].includes(
        this.game.observer_level
      )
    ) {
      buttons.push(
        <DeleteButton
          key={`game-delete-${this.game.game_id}`}
  ...
  get(name) {
    ...
    if (name === "rights") {
      ...
      if (this.game.observer_level) {
        ...
        else levelName = "observer";
        elements.push(
          <p key={0}>
            <strong>Observer right:</strong>
            <br />
            {levelName}
          </p>
        );
      ...
      if (this.game.controlled_powers && this.game.controlled_powers.length) {
        ...
        elements.push(
          <div key={1}>
            <strong>
              Currently handled power{powers.length === 1 ? "" : "s"}
            </strong>
          </div>
        );
        for (let power of powers) elements.push(<div key={power}>{power}</div>);
      }
    ...
    if (name === "join") return this.getJoinUI();
    if (name === "actions") return this.getActionButtons();
    if (name === "game_id") {
      ...
      return (
        <div>
          <div>
            <strong>{this.game.game_id}</strong>
          </div>
          <div>({dateString})</div>
          <div>
            <em>{this.game.map_name}</em>
          </div>
        </div>
      );
    }
