...
export class SvgStandard extends React.Component {
  ...
  handleClickedID(id) {
    ...
    if (orderBuilding.path.length >= stepLength)
      throw new Error(
        `Order building: current steps count (${
          orderBuilding.path.length
        }) should be less than  expected steps count (${stepLength}) (${orderBuilding.path.join(
          ", "
        )}).`
      );
    ...
    const value = UTILS.javascript.getTreeValue(
      this.props.game.ordersTree,
      testedPath
    );
    if (value !== null) {
    ...
    if (
      ...
      if (validLocations.length === validLocationsNoProvinceName.length) {
        /* We have not found province name. */ isACoast = false;
      }
      if (isACoast) {
        /* We want to choose location in a coastal province. Let's remove province name. */ validLocations =
          validLocationsNoProvinceName;
      }
    }
    ...
    if (lengthAfterClick === stepLength && orderBuildingType === "M") {
      ...
      const moveTypes = UTILS.javascript.getTreeValue(
        this.props.game.ordersTree,
        moveOrderPath
      );
      if (moveTypes !== null) {
        if (moveTypes.length === 2 && this.props.onSelectVia) {
          /* This move can be done either regularly or VIA a fleet. Let user choose. */ return this.props.onSelectVia(
            validLocations[0],
            orderBuilding.power,
            orderBuilding.path
          );
  ...
  getNeighbors(extraLocation) {
    const selectedPath = [this.props.orderBuilding.type].concat(
      this.props.orderBuilding.path
    );
    if (extraLocation) selectedPath.push(extraLocation);
    const possibleNeighbors = UTILS.javascript.getTreeValue(
      this.props.game.ordersTree,
      selectedPath
    );
    const neighbors = possibleNeighbors
      ? possibleNeighbors.map((neighbor) => parseLocation(neighbor))
      : [];
  ...
  render() {
    ...
    /* Current phase.  const current_phase =
      game.phase[0] === "?" || game.phase === "COMPLETED"
        ? "FINAL"
        : game.phase;
     Notes. */ const nb_centers = [];
    ...
    /* Sort nb_centers by descending number of centers. */ nb_centers.sort(
      (a, b) => {
        return -(a[1] - b[1]) || a[0].localeCompare(b[0]);
      }
    );
    /*const nb_centers_per_power = nb_centers
      .map((couple) => couple[0] + ": " + couple[1])
      .join(" ");
    
    const note = game.note;
     Adding units, influence and orders. */ const renderedUnits = [];
    ...
    for (let power of Object.values(game.powers))
      if (!power.isEliminated()) {
        ...
        if (orders) {
          const powerOrders =
            (orders &&
              orders.hasOwnProperty(power.name) &&
              orders[power.name]) ||
            [];
          for (let order of powerOrders) {
            ...
            if (tokens[2] === "H") {
              ...
              const destLoc =
                tokens[
                  tokens.length - (tokens[tokens.length - 1] === "VIA" ? 2 : 1)
                ];
              renderedOrders.push(
                <Move
                  key={order}
    ...
    if (this.props.orderBuilding && this.props.orderBuilding.path.length) {
      ...
      if (!clicked)
        throw new Error(
          `Unknown path (${clickedID}) for province (${clicked}).`
        );
      ...
      if (neighbors) {
        for (let neighbor of neighbors) {
          ...
          if (!neighborID)
            throw new Error(
              `Unknown neoghbor path (${neighborID}) for province (${neighbor}).`
            );
          classes[neighborID] = neighborProvince.isWater()
            ? "provinceBlue"
            : "provinceGreen";
        }
      }
    ...
    return (
      ...
      >
        ...
        <g id="MapLayer" transform="translate(-195 -170)">
          ...
          />{" "}
          <path
            className={classes["unplayable_water"]}
          ...
          />{" "}
          <path
            className={classes["denmark_water"]}
          ...
          />{" "}
          <path
            className={classes["constantinople_water"]}
        ...
        <g className={classes["BriefLabelLayer"]} id="BriefLabelLayer">
          {" "}
          <text x="649.4" y="924.2">
            SWI
          </text>{" "}
          <text className="labeltext18" x="849.4" y="1100.6">
            ADR
          </text>{" "}
          <text x="1085.7" y="1296.4">
            AEG
          </text>{" "}
          <text className="labeltext18" x="932.4" y="1164.4">
            ALB
          </text>{" "}
          <text x="1368.8" y="1115.5">
            ANK
          </text>{" "}
          <text className="labeltext18" x="832.3" y="1151.7">
            APU
          </text>{" "}
          <text x="1603" y="1145.3">
            ARM
          </text>{" "}
          <text x="827.2" y="660.2">
            BAL
          </text>{" "}
          <text x="1077.8" y="59.3">
            BAR
          </text>{" "}
          <text className="labeltext18" x="591.8" y="796.2">
            BEL
          </text>{" "}
          <text className="labeltext18" x="760" y="728">
            BER
          </text>{" "}
          <text x="1319.8" y="1034.6">
            BLA
          </text>{" "}
          <text className="labeltext18" x="800.4" y="855.8">
            BOH
          </text>{" "}
          <text className="labeltext18" x="430" y="866.4">
            BRE
          </text>{" "}
          <text className="labeltext18" x="953.7" y="964.3">
            BUD
          </text>{" "}
          <text className="labeltext18" x="1021.8" y="1109.1">
            BUL
          </text>{" "}
          <text className="labeltext18" x="530.1" y="915.4">
            BUR
          </text>{" "}
          <text className="labeltext18" x="457.7" y="479">
            CLY
          </text>{" "}
          <text x="1202.8" y="1177.2">
            CON
          </text>{" "}
          <text x="718.2" y="584.3">
            DEN
          </text>{" "}
          <text x="1164.4" y="1343.3">
            EAS
          </text>{" "}
          <text className="labeltext18" x="496" y="500.3">
            EDI
          </text>{" "}
          <text x="466.2" y="762.6">
            ENG
          </text>{" "}
          <text x="1030.3" y="347">
            FIN
          </text>{" "}
          <text className="labeltext18" x="940.9" y="840.9">
            GAL
          </text>{" "}
          <text className="labeltext18" x="447" y="972.8">
            GAS
          </text>{" "}
          <text className="labeltext18" x="981.4" y="1160.2">
            GRE
          </text>{" "}
          <text x="929.6" y="538.9">
            BOT
          </text>{" "}
          <text x="525.8" y="1107">
            LYO
          </text>{" "}
          <text x="654.2" y="665.4">
            HEL
          </text>{" "}
          <text className="labeltext18" x="608.8" y="749.3">
            HOL
          </text>{" "}
          <text x="894.1" y="1251.7">
            ION
          </text>{" "}
          <text x="318" y="711.4">
            IRI
          </text>{" "}
          <text className="labeltext18" x="691.9" y="753.6">
            KIE
          </text>{" "}
          <text className="labeltext18" x="500.3" y="734.4">
            LON
          </text>{" "}
          <text x="1030.3" y="640.8">
            LVN
          </text>{" "}
          <text className="labeltext18" x="449.2" y="564.1">
            LVP
          </text>{" "}
          <text className="labeltext18" x="572.6" y="962.2">
            MAR
          </text>{" "}
          <text x="153.3" y="815.3">
            MAO
          </text>{" "}
          <text x="1568.9" y="572.6">
            MOS
          </text>{" "}
          <text className="labeltext18" x="681.2" y="872.8">
            MUN
          </text>{" "}
          <text x="240.6" y="1317.7">
            NAF
          </text>{" "}
          <text className="labeltext18" x="832.3" y="1211.3">
            NAP
          </text>{" "}
          <text x="191.6" y="268.2">
            NAO
          </text>{" "}
          <text x="595.5" y="512">
            NTH
          ...
          <text x="664.2" y="161.8">
            NWG
          </text>{" "}
          <text className="labeltext18" x="481.1" y="892">
            PAR
          </text>{" "}
          <text className="labeltext18" x="489.6" y="802.5">
            PIC
          </text>{" "}
          <text className="labeltext18" x="628" y="1004.8">
            PIE
          </text>{" "}
          <text transform="translate(175.1 1102.1) rotate(-60)" x="0" y="0">
            POR
          </text>{" "}
          <text className="labeltext18" x="938.8" y="687.6">
            PRU
          </text>{" "}
          <text
          ...
          <text className="labeltext18" x="632.2" y="826">
            RUH
          </text>{" "}
          <text className="labeltext18" x="1036.7" y="1015.4">
            RUM
          </text>{" "}
          <text className="labeltext18" x="964.3" y="1100.6">
            SER
          </text>{" "}
          <text x="1479.3" y="937.7">
            SEV
          </text>{" "}
          <text className="labeltext18" x="791.9" y="779.1">
            SIL
          </text>{" "}
          <text transform="translate(754.5 571.2) rotate(75)" x="0" y="0">
            SKA
          </text>{" "}
          <text x="1194.2" y="1247.5">
            SMY
          </text>{" "}
          <text x="302.3" y="1119.7">
            SPA
          </text>{" "}
          <text x="1473.1" y="217.1">
            STP
          </text>{" "}
          <text x="845.1" y="391.7">
            SWE
          </text>{" "}
          <text x="1639.2" y="1292.2">
            SYR
          </text>{" "}
          <text className="labeltext18" x="826" y="966.5">
            TRI
          </text>{" "}
          <text x="634.4" y="1351.8">
            TUN
          </text>{" "}
          <text className="labeltext18" x="706.5" y="1069">
            TUS
          </text>{" "}
          <text className="labeltext18" x="717.4" y="936.7">
            TYR
          </text>{" "}
          <text x="710" y="1209.1">
            TYS
          ...
          <text className="labeltext18" x="749.3" y="962.2">
            VEN
          </text>{" "}
          <text className="labeltext18" x="834.5" y="919.6">
            VIE
          </text>{" "}
          <text className="labeltext18" x="436.4" y="719.5">
            WAL
          </text>{" "}
          <text className="labeltext18" x="938.8" y="783.4">
            WAR
          </text>{" "}
          <text x="400.2" y="1209.1">
            WES
          </text>{" "}
          <text className="labeltext18" x="496" y="653.5">
            YOR
          </text>{" "}
        </g>{" "}
        <rect
