...
export class Channel {
  ...
  localJoinGame(joinParameters) {
    ...
    if (this.game_id_to_instances.hasOwnProperty(joinParameters.game_id)) {
      ...
      if (joinParameters.power_name)
        return this.game_id_to_instances[joinParameters.game_id].get(
          joinParameters.power_name
        );
  ...
  _req(name, forcedParameters, localChannelFunction, parameters, game) {
    ...
    if (level === STRINGS.GAME) {
      ...
      if (game)
        throw new Error(
          "A game object should not be provided for a non-game request."
        );
  ...
  getReceptionAddresses(parameters) {
    ...
    const game = this.game_id_to_instances[parameters.game_id];
    if (!game) {
