...
export const NOTIFICATION_MANAGERS = {
  ...
  powers_controllers: function (game, notification) {
    if (
      ...
      notification.powers[game.local.role] !==
        game.local.getRelatedPower().getController()
    ) {
      game.channel.game_id_to_instances[game.local.game_id].remove(
        game.local.role
      );
      if (!game.channel.game_id_to_instances[game.local.game_id].size())
        delete game.channel.game_id_to_instances[game.local.game_id];
    } else {
      game.local.updatePowersControllers(
        notification.powers,
  ...
  omniscient_updated: function (game, notification) {
    ...
    const oldGame = channel.game_id_to_instances[game.local.game_id].remove(
      game.local.role
    );
    oldGame.client = null;
  ...
  power_orders_flag: function (game, notification) {
    game.local.getPower(notification.power_name).order_is_set =
      notification.order_is_set;
  },
  ...
  handleNotification: function (connection, notification) {
    if (!NOTIFICATION_MANAGERS.hasOwnProperty(notification.name))
      throw new Error(
        "No notification handler available for notification " +
          notification.name
      );
    ...
    if (!connection.channels.hasOwnProperty(notification.token))
      throw new Error(
        "Unable to find channel related to notification " + notification.name
      );
      ...
      if (gis) {
        if (typeof gis.has === "function" && typeof gis.get === "function") {
          if (gis.has(notification.game_role)) {
            ...
            console.log(
              "roles for this game (Map):",
              Array.from(gis.keys && (gis.keys() || []))
            );
            throw new Error(
        ...
        console.log(
          ...
          objectToNotify.game_id_to_instances
        );
        throw new Error(
          "Unable to find game instance related to notification " +
            notification.name +
